...
describe("Signup Test", () => {
    ...
    context('Valid Credentials', () => {
        it('TC01: Should signup with valid credentials', () => {
            ...
            cy.get('#username', { timeout: 10000 }).typeText(username);
            cy.get('#fullName', { timeout: 10000 }).typeText(fullname);
            cy.get('#birthday', { timeout: 10000 }).typeText(DOB); //trong HTML, trình duyệt web mặc định yêu cầu định dạng ngày tháng phải là YYYY-MM-DD
            cy.get('#mobile', { timeout: 10000 }).typeText(phone);
            cy.get('#email', { timeout: 10000 }).typeText(email);
            cy.get('#cityId', { timeout: 10000 }).select(city);
            cy.get('#districtId', { timeout: 10000 }).select(district);
            cy.get('#address', { timeout: 10000 }).typeText(address);
            cy.get('#password', { timeout: 10000 }).typeText(password);
            cy.get('#rePassword', { timeout: 10000 }).typeText(repassword);
    ...
    context('Empty Fields', () => {
        it('TC02: Should not signup with empty username', () => {
            ...
            cy.get('#fullName', { timeout: 10000 }).typeText(fullname);
            cy.get('#birthday', { timeout: 10000 }).typeText(DOB);
            cy.get('#mobile', { timeout: 10000 }).typeText(phone);
            cy.get('#email', { timeout: 10000 }).typeText(email);
            cy.get('#cityId', { timeout: 10000 }).select(city);
            cy.get('#districtId', { timeout: 10000 }).select(district);
